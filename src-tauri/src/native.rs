//! Background worker. On Windows it drives both drag-to-snap (Win32 WinEvent move/size hooks + a
//! layered preview window) and foreground tracking (`EVENT_SYSTEM_FOREGROUND`). Drag-snap has no
//! portable macOS equivalent, so on macOS the worker is reduced to a light foreground poll that
//! keeps `last_active` on the user's real window (so the in-app actions have a target); the
//! keyboard-driven tiling that is the cross-platform core needs neither.

#[cfg(windows)]
pub use win::spawn_worker;
...
/// Other platforms: no background worker.
...
mod win {
    ...
    unsafe extern "system" fn winevent_proc(
        ...
        match event {
            EVENT_SYSTEM_FOREGROUND => {
                if winmgr::is_manageable(hwnd.0 as isize) {
            ...
            EVENT_SYSTEM_MOVESIZEEND => {
                ...
                if enabled && dh != 0 {
                    if let Some(z) = zone {
                        winmgr::apply_visible_rect(dh, z);
                    }
                }
            }
            _ => {}
